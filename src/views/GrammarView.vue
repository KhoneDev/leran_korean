...
import { getGrammarByLevel, type GrammarPoint } from '@/data/grammar'
import { getSentencesByLevel, type SentenceExercise } from '@/data/sentences'
...
const tabs = [
  { key: 'grammar' as const, icon: '📖', label: 'ไวยากรณ์' },
  { key: 'build' as const, icon: '🧩', label: 'สร้างประโยค' },
  { key: 'order' as const, icon: '🔢', label: 'เรียงประโยค' },
]
</script>
...
<template>
  <div class="max-w-3xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
    ...
    <div class="text-center mb-8">
      <h1 class="text-2xl sm:text-3xl font-bold text-slate-800 mb-2">📖 ไวยากรณ์เกาหลี</h1>
      <p class="text-sm sm:text-base text-slate-500">
        Level {{ userStore.progress.currentLevel }} — เรียนรู้โครงสร้างประโยค
      </p>
    </div>
    <!-- Tabs -->
    ...
    <div v-if="activeTab === 'grammar'">
      ...
      <div v-else class="text-center py-12 text-slate-400">
        ...
        <p class="text-lg">ยังไม่มีเนื้อหาไวยากรณ์สำหรับ Level นี้</p>
      </div>
    </div>
    <!-- Build Tab -->
    <div v-if="activeTab === 'build'">
      <div v-if="buildExercises.length > 0 && currentSentence">
        <div class="flex items-center justify-between mb-4">
          <span class="text-sm text-slate-500">
            ข้อ {{ currentSentenceIndex + 1 }} / {{ buildExercises.length }}
          </span>
        </div>
      ...
      <div v-else class="text-center py-12 text-slate-400">
        <div class="text-4xl mb-3">🧩</div>
        <p class="text-lg">ยังไม่มีแบบฝึกหัดสร้างประโยค</p>
      </div>
    </div>
    <!-- Order Tab -->
    <div v-if="activeTab === 'order'">
      <div v-if="orderExercises.length > 0 && currentSentence">
        <div class="flex items-center justify-between mb-4">
          <span class="text-sm text-slate-500">
            ข้อ {{ currentSentenceIndex + 1 }} / {{ orderExercises.length }}
          </span>
        </div>
      ...
      <div v-else class="text-center py-12 text-slate-400">
        <div class="text-4xl mb-3">🔢</div>
        <p class="text-lg">ยังไม่มีแบบฝึกหัดเรียงประโยค</p>
      </div>
    </div>
  </div>
</template>
